...
trait HasHttpRequests
{
    ...
    public function getHttpClient(): ClientInterface
    {
        if (!($this->httpClient instanceof ClientInterface)) {
            if (property_exists($this, 'app') && $this->app instanceof Container && in_array('http_client', $this->app->keys())) {
    ...
    public function performRequest($url, $method = 'GET', $options = []): ResponseInterface
    {
        ...
        $options = array_merge(static::$defaults, $options, ['handler' => $this->getHandlerStack()]);
        ...
        $response = $this->getHttpClient()->request($method, ltrim($url, '/'), $options);
    ...
    protected function fixJsonIssue(array $options): array
    {
        if (isset($options['json']) && is_array($options['json'])) {
            ...
            if (empty($options['json'])) {
                ...
                $options['body'] = Utils::jsonEncode($options['json'], \JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES);
            }
